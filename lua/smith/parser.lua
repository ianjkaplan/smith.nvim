---@class SmithParser
---@field state SmithParserState
local M = {}

---@class SmithParserState
...
local function format_thinking_delta(data)
  ...
  return lines, true -- true = should replace previous thinking block
end
...
local function format_thinking_completed()
  ...
  -- Clear thinking buffer for next round
...
local function format_assistant_delta(data)
  local lines = {}
  if data.text then
    -- For streaming text, just append the delta
    table.insert(lines, data.text)
  end
  return lines, true
end

---@alias TypeHandler fun(data: table, result: ParseResult): nil

---Dispatch table for message type handlers
...
---@class ParseResult
---@field lines string[] Formatted lines to display
---@field replace_thinking boolean Whether to replace the previous thinking block
---@field is_streaming boolean Whether this is a streaming update
---@field type string|nil The message type
...
---@class StreamState
---@field lines string[] All accumulated lines
---@field thinking_start number|nil Line index where current thinking block starts
---@field assistant_streaming_start number|nil Line index where streaming assistant text starts
---@field assistant_buffer string Accumulated assistant streaming text
...
---Helper to remove lines from thinking_start onwards
---@param stream_state StreamState
...
---@alias StreamUpdateHandler fun(stream_state: StreamState, result: ParseResult): nil

---Dispatch table for stream state update handlers
...
---Determine the handler key for a parse result
...
---Parse a line and update stream state (for buffer management)
---@param stream_state StreamState
---@param line string Raw JSON line
---@return StreamState, boolean updated Whether lines were updated
function M.parse_and_update(stream_state, line)
  ...
  stream_update_handlers[key](stream_state, result)

  return stream_state, true
end

---Parse the entire agent stream output
---@param raw_lines string[] Array of raw JSON lines
---@return string[] Formatted lines for display
function M.parse_agent_stream(raw_lines)
  M.reset()
